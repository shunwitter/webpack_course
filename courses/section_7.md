...
問題ないように見えますが、ウェブサイトは最終的にウェブサーバーが`http`プロトコルを使用して配信されます。ローカル環境でも同じように`http`を使用するにはローカル環境でもサーバーを立ち上げる必要があります。ローカルサーバーのメリットを簡単にまとめました。

- 1. 同じプロトコルなのでリリース後の問題が少ない
- 2. ライブリロードが便利
- 3. ルートパス`/`が使える
...
1番目のメリット`同じプロトコルを利用する`が確認できたかと思います。
